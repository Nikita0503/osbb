...
const PageHeaderClickable = props => {
  return (
    <Header
      leftComponent={<HamburgerMenu style={{width: 50, height: 50}} navigation={props.navigation} />}
  centerComponent={<TouchableOpacity onPress={()=>{
    props.setCurrentWorkPeriod(props.workPeriods[props.workPeriods.length - 1])
  }}><Text style={{ color: "#fff", marginRight: 65}}>{props.title}</Text></TouchableOpacity>}
      rightComponent={
        <View style={{flexDirection: 'row'}}>
          ...
          <TouchableOpacity onPress={() => props.navigation.navigate('Support')}>         
            <Image style={styles.icon} source={require('../images/ic_headphones.png')} />  
          </TouchableOpacity>
        </View>
      }
