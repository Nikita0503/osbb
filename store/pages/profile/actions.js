...
export const sendNewPhoto = (formdata, accountId, osbbId, workPeriods, token) => {
  return async dispatch => {
      try {
          const newPhotoPromise = await fetch('https://app.osbb365.com/api/upload/photo?accountId=' 
            + accountId + '&osbbId=' 
          ...
          const changePhotoPromise = await fetch('https://app.osbb365.com/api/user/me', {
            method: 'put',
            headers: {
              Authorization: 'Bearer ' + token,
...
export const sendNewPassword = (oldPassword, newPassword, newRepeatPassword, token) => {
  return async dispatch => {
      try {
          const newPasswordPromise = await fetch('https://app.osbb365.com/api/user/me/password', {
            method: 'PUT',
            headers: {
              Accept: 'application/json',
              'Content-Type': 'application/json',
