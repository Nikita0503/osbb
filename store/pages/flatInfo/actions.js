        ...
        if (e.data.substring(0, 2) == '42') {
          ...
          for (var i = accountIds.length - 1; i >= 0; i--) {
            for (var j = data[1].UserAccounts.length - 1; j >= 0; j--) {
              if (
                accountIds[i].number == data[1].UserAccounts[j].number
              ) {
...
export const fetchFlatInfoCounters = (accountIds, osbbId, workPeriods, index, token) => {
  return async dispatch => {
    try {
...
export const fetchFlatInfoLodgerData = (accountIds, osbbId, workPeriods, index, token) => {
  return async dispatch => {
    try {
...
export const fetchFlatInfoParameters = (accountIds, osbbId, workPeriods, index, token) => {
  return async dispatch => {
    try {
...
export const fetchFlatInfoPrivileges = (accountIds, osbbId, workPeriods, token) => {
  return async dispatch => {
    try {
...
export const fetchFlatInfoContracts = (accountIds, osbbId, workPeriods, token) => {
  return async dispatch => {
    try {
      var ws = new WebSocket(
        'wss://app.osbb365.com/socket.io/?auth_token=' +
      ...
      ws.onmessage = e => {
        if (e.data.substring(0, 2) == '42') {
          ...
          var myObj = JSON.parse(myObjStr);
          var data = JSON.parse(myObj);
          fetchFlatInfoContractsByAccountId(0, data[1].UserAccounts, accountIds, osbbId, token);
          ws.close();
        }
      };
    } catch (error) {
...
export const fetchFlatInfoContractsByAccountId = (index, 
    userAccounts, 
    accountIds, 
    osbbId, 
    token) => {
  return async dispatch => {
    try {
      ...
      if (index != accountIds.length - 1) {
        index++;
        fetchFlatInfoContractsByAccountId(index, 
          userAccounts, 
          accountIds, 
          osbbId, 
          token);
