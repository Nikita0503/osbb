...
export const AUTH_CHANGE_HAS_CAMERA_PERMISSION =
  ...
  return async (dispatch) => {
    try {
      const tokenPromise = await fetch(
        "https://app.osbb365.com/register/tenant/mobile",
        {
          method: "POST",
          headers: {
            Accept: "application/json",
      ...
      if (error.message == "JSON Parse error: Unrecognized token '<'") {
        ...
        error.message == 'JSON Parse error: Unexpected identifier "OK"'
      ) {
        Alert.alert(
...
export const signInDevice = (token, navigation) => {
  return async (dispatch) => {
    try {
      const tokenPromise = await fetch(
        "https://app.osbb365.com/login/tenant/mobile",
        {
          method: "POST",
          headers: {
            Accept: "application/json",
